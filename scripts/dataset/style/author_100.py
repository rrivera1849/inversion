...
def filter_by_lengths(row):
    ...
    indices_to_keep = []
    for i in range(N):
        if row["num_tokens"][i] >= 128 and row["num_tokens"][i] <= 512:
...
def main():
    ...
    df = pd.read_json(os.path.join(base_path, "train.jsonl"), lines=True)
    ...
    train.to_json(os.path.join(base_path, "author_100", "train.jsonl"), lines=True, orient="records")
    valid.to_json(os.path.join(base_path, "author_100", "valid.jsonl"), lines=True, orient="records")
    queries.to_json(os.path.join(base_path, "author_100", "test.jsonl"), lines=True, orient="records")
